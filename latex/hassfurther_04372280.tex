...
Liegen die Werte hingegen auf dem Heap, muss die ``clone''-Methode für den Typen implementiert werden. Hierdurch ist die Kopie explizit und kopiert standardmäßig ebenfalls die Daten auf dem Heap. Durch das Anpassen der Methode können aber z.B. auch nur die Stackwerte kopiert werden und ein Referenzzähler implementiert werden. Die ``clone''-Methode erlaubt somit flexible Strukturen \cite[S. 260 ff.]{SK19}. Bei einer Zuweisung wird wie bereits gezeigt das Ownership abgegeben, wenn nicht die ``clone''-Methode aufgerufen wird.
...
In Rust sieht die Implementierung folgendermaßen aus. Die Ausprägung Add benötigt für die Werte immer den Wrapper \codeword{Box} der die Daten auf den Heap speichert. Zur Compilezeit lässt sich die Größe auf dem Stack nicht bestimmen, da nicht bestimmt ist, wie viele Ebenen Add beinhaltet \cite{RustRecursiveTypes}.
