...
Unterschied 0.1+0.2 nochmal mit precision num erwähnen und implementieren! sqrt unterschied erklären
...
Minimale Unterschiede in der Zusammenfassung. Wobei normaler Output von Sympy nicht mit Datenstruktur in Speicher übereinstimmt. Mit sympy.srepr ist z.B. der Term $x^a/x$ als ``Mul(Pow(Symbol('x'), Integer(-1)), Pow(Symbol('x'), Symbol('a')))'' gespeichert.
...
Unterschied auch wieder nur visuell. $sqrt(17)$ ist mit sympy.srepr als ``Pow(Integer(17), Rational(1, 2))'' intern abrufbar.
...
Im großen und ganzen wieder ähnliche Ergebnisse. Bei SymPy wird in Termen, mit Klammern und selber Variable bei den Summanden diese zusätlich ausgeklammert.
